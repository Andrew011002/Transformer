...
In traditional sequence to sequence (seq2seq) models used commonly for NMT, they follow a encoder-decoder design pattern in which both modules are RNNs (either LSTMs or GRUs). The encoder takes an input sequence and encodes it word by word, while simultaneously learning contextual information about parts of the sequence to store as its internal state until it has encoded the entire sequence. Recall, the encoder is an LSTM/GRU, so not all contextual information from the sequence is stored in its internal state and may be forgotten by its gates as it processes. Continuing, when this process is complete, the encoder passes a fixed length vector to the decoder representing the contextual information of the input sequence (i.e. the most recent learned internal state of the encoder). During training, the decoder learns to predict the translated word based on words it predicted previously and the last internal state of the encoder (Note: correct translation are enforced only during training to help the decoder learn). Typically speaking, the decoder is fed a start of sentence word (e.g. <sos>) that initializes the predicting process and learns to stop predicting using an end of sentence word (e.g. <eos>) both provided at the beginning and ending of the translated sequence respectively. During inference, the same process follows, except it's only given the start of sentence word and auto-regressively predicts word by word based on what it has learned during training. This process was great for NMT, but it had an issue that we previously mentioned and that's the fact the context vector from the encoder is fixed, meaning it can't remember all the parts of the input sequence to assist the decoder in translation. As a resolution of to this issue, the process of understanding the overall context of an entire sequence was created and is known as the Attention Mechanism.
